...
from backend.shared.user_config import (
    DEFAULT_ACTION_BAR_SLOT,
    action_bar_slot_to_vk,
    load_or_create_user_behavior_config,
    resolve_user_behavior_config_path,
)
...
def apply_action_bar_slot_override(
    steps: list[dict[str, Any]],
    action_bar_slot: int,
) -> tuple[list[dict[str, Any]], bool]:
    updated_steps = [dict(step) for step in steps]
    if ACTION_SLOT_STEP_INDEX >= len(updated_steps):
        return updated_steps, False

    target_step = dict(updated_steps[ACTION_SLOT_STEP_INDEX])
    if target_step.get("action") != "tap":
        return updated_steps, False

    target_step["vk"] = action_bar_slot_to_vk(action_bar_slot)
    updated_steps[ACTION_SLOT_STEP_INDEX] = target_step
    return updated_steps, True
...
def _legacy_command_watcher_v2(
    command_path: Path,
    main_thread_id: int,
    running_event: threading.Event,
    shutdown_event: threading.Event,
...
def _legacy_command_watcher_v3(
    command_path: Path,
    running_event: threading.Event,
    shutdown_event: threading.Event,
    status_path: Path,
...
def main() -> None:
    ...
    user_config_path = resolve_user_behavior_config_path(PROJECT_ROOT)
    ...
    try:
        user_behavior, user_config_created = load_or_create_user_behavior_config(user_config_path)
    except Exception as exc:
        raise RuntimeError(f"Failed to load user config {user_config_path}: {exc}") from exc
    action_bar_slot = user_behavior.double.action_bar_slot
    ...
    config.steps, action_slot_override_applied = apply_action_bar_slot_override(
        config.steps,
        action_bar_slot,
    )
    ...
    runtime_status["user_config"] = str(user_config_path)
    runtime_status["action_bar_slot"] = action_bar_slot
    ...
    publish_runtime_status(
        ...
        runtime_status,
        message=(
            f"User config created: {user_config_path}"
            if user_config_created
            else f"User config: {user_config_path}"
        ),
        level="info",
    )
    if action_slot_override_applied:
        publish_runtime_status(
            status_path,
            status_lock,
            runtime_status,
            message=f"Action slot: {action_bar_slot}",
            level="info",
        )
    elif action_bar_slot != DEFAULT_ACTION_BAR_SLOT:
        publish_runtime_status(
            status_path,
            status_lock,
            runtime_status,
            message=(
                f"Configured action slot {action_bar_slot} was not applied "
                "because the current step layout did not match."
            ),
            level="warn",
        )
    if active_profile_path:
