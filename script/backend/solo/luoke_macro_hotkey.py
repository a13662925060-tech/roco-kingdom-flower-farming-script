...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError(f"加载用户配置失败 {user_config_path}：{exc}") from exc
    action_bar_slot = user_behavior.solo.action_bar_slot
    ...
    publish_runtime_status(
        ...
        runtime_status,
        message=(
            f"已生成用户配置：{user_config_path}"
            if user_config_created
            else f"用户配置：{user_config_path}"
        ),
        level="info",
    )
    if action_slot_override_applied:
        publish_runtime_status(
            ...
            status_lock,
            runtime_status,
            message=f"动作栏位：{action_bar_slot}",
            level="info",
        )
        ...
        publish_runtime_status(
            ...
            status_lock,
            runtime_status,
            message=f"用户配置中的动作栏位 {action_bar_slot} 未应用：当前步骤结构不匹配。",
            level="warn",
        )
    if active_profile_path:
        publish_runtime_status(
